...
Это позволяет разработчикам по-прежнему писать выразительный, легкочитаемы и легко понимаемый код улучшая при этом его производительность после транспиляции.
...
Поразмыслив немного - я согласился с ними, и пришел к пониманию, что необходимо изучить Babel и написать на нем набор плагинов, которые бы исправляли проблемы с производительностью в npm зависимостях!
...
### `name`: string, по-усолчанию: `unamed_config`
...
### `target`: 'node' | 'custom', по-усолчанию: 'node'
...
### `customTransformations`: string[], по-усолчанию: []
...
### `unsafeTransformations`: boolean, по-усолчанию: false

> Внимание!

Хоть по-умолчанию этот параметр и устанавливается в `false` настоятельно рекомендую устанавливать его в `true`.

Данным параметром в пресете помечены исключительно трансформации для цепочек вызовов циклиxеских методов для массивов.
...
### `verbose`: boolean, по-усолчанию: false

Параметр отвечает за вывод в консоль предупреждений во время трансформаций.

### `useStatsServer`: boolean | object, по-усолчанию: false

Во время работы пресета можно собирать всю статистику по производимым трансформациям в модулях.
...
Нет смысла ждать окончания нативно реализованного цикла для получения `values` из объекта
...
Это может добавить несколько процентов производительности, учитывая то, что `const` и `let` необходимо создать для своей работы контекст, а это - дополнительные затраты времени.
...
Идею с данной трансформацией, а так много еще полезных советов предложил мне [Виктор Хомяков](https://github.com/victor-homyakov)

## Поиск и устранение проблем

Может случиться трагедия - один из пакетов в ваших зависимостях плодит массивы с дырками (надеюсь не ваш код!?) - в таком случае трасформации могут выдавать не тот результат который ожидается (все зависит от методов - они оказывается по-разному обрабатывают дырки)!

Встает вопрос в обнаружении данного нерадивого разработчика, его пакета и исключение его и его пакета из обработки пресетом.
...
Для начала мы можем составить список пакетов, используемых нашим приложением выделив из полного пути модулей лишь имена пакетов, а затем использовав [правило половинного деления](https://mybiblioteka.su/9-108554.html), можно последовательно уточняя список разрешенных пакетов,
...
найти проблемные и уже после перебирая все модули проблемных пакетов, найти действительно дефектные и добавить их в список `exclude` для лоадера
